...
/**
 * Data Access Object for the trivia database. Handles initial table creation
 * and population.
 * 
 * @author Dan Ruscoe
 */
public class TriviaDAO extends SQLiteOpenHelper {
	...
	private static final String DROP_TABLE_USER_PREFS = "DROP TABLE IF EXISTS "
			+ UserPrefsData.TABLE_NAME;

	private static final String CREATE_TABLE_USER_PREFS = "CREATE TABLE "
			+ UserPrefsData.TABLE_NAME + " (" + _ID + " INTEGER PRIMARY KEY, "
			+ UserPrefsData.SOUND + " INTEGER DEFAULT 1" + ");";
	
	private static final String DROP_TABLE_CATEGORIES = "DROP TABLE IF EXISTS "
			+ CategoryData.TABLE_NAME;

	private static final String CREATE_TABLE_CATEGORIES = "CREATE TABLE "
			+ CategoryData.TABLE_NAME + " (" + _ID + " INTEGER PRIMARY KEY, "
			+ CategoryData.NAME + " TEXT" + ");";
	
	private static final String DROP_TABLE_QUESTIONS = "DROP TABLE IF EXISTS "
			+ QuestionData.TABLE_NAME;

	private static final String CREATE_TABLE_QUESTIONS = "CREATE TABLE "
			+ QuestionData.TABLE_NAME + " (" + _ID
			+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + QuestionData.CATEGORY_ID
			+ " INTEGER, " + QuestionData.DIFFICULTY + " INTEGER, "
			+ QuestionData.TEXT + " TEXT," + QuestionData.DESCRIPTION + " TEXT"
			+ ");";

	private static final String DROP_TABLE_ANSWERS = "DROP TABLE IF EXISTS "
			+ AnswerData.TABLE_NAME;

	private static final String CREATE_TABLE_ANSWERS = "CREATE TABLE "
			+ AnswerData.TABLE_NAME + " (" + _ID
			+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + AnswerData.QUESTION_ID
			+ " INTEGER, " + AnswerData.TEXT + " TEXT," + AnswerData.CORRECT
			+ " INTEGER" + ");";
	
	private static final String DROP_TABLE_SCORES = "DROP TABLE IF EXISTS "
			+ ScoreData.TABLE_NAME;

	private static final String CREATE_TABLE_SCORES = "CREATE TABLE "
			+ ScoreData.TABLE_NAME + " (" + ScoreData.CATEGORY_ID
			+ " INTEGER PRIMARY KEY, " + ScoreData.QUESTIONS_ANSWERED
			+ " INTEGER," + ScoreData.CORRECT_ANSWERS + " INTEGER" + ");";
	...
	private void onDrop(SQLiteDatabase db) {
		...
		Log.d(Constants.APP_LOG_NAME, "Dropping database tables");
		
		db.execSQL(DROP_TABLE_USER_PREFS);
		db.execSQL(DROP_TABLE_CATEGORIES);
		db.execSQL(DROP_TABLE_QUESTIONS);
		db.execSQL(DROP_TABLE_ANSWERS);
		db.execSQL(DROP_TABLE_SCORES);
	}

	@Override
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		Log.d(Constants.APP_LOG_NAME, "Upgrading database from version oldVersion[" + oldVersion + "] to newVersion[" + newVersion + "]");
		
		// today we drop em all.
		// maybe we could just drop some in the future.
	...
	private void importQuestionsData(SQLiteDatabase db, int resourceId) {
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if (recordData.length < 7) {
				Log.e(Constants.APP_LOG_NAME,
						"Question record too short at index " + i
								+ " while importing resource ID " + resourceId);
			}
			...
			// There are three possible answers per question. They are imported
			// here.
	...
	/**
	 * Stores a question answer record in the database. Each question has
	 * multiple answers, with only one being correct.
	 * 
	 * @param SQLiteDatabase
	 *            db - The database connection.
	 ...
	 */
	private void importAnswerData(SQLiteDatabase db, long questionId,
			String answerText, boolean correct) {
		ContentValues values = new ContentValues();
	...
	/**
	 * Reads rows from a CSV data file and returns each record as a string
	 * array.
	 * 
