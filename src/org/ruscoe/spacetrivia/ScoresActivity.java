...
/**
 * Displays a list of trivia categories mapped to the user's highest score in
 * each.
 * 
 * @author Ruscoe
 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < categories.size(); i++) {
			...
			if (currentScore != null) {
				Log.i(Constants.APP_LOG_NAME, "Found score for category ID "
						+ currentCategory.getCategoryId());

				currentScore.setCategoryName(currentCategory.getName());
			} else {
				Log.i(Constants.APP_LOG_NAME, "No score found for category ID "
						+ currentCategory.getCategoryId());
