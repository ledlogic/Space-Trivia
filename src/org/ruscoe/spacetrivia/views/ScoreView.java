...
/**
 * Generates a view to display a score in a list. Controls the order and
 * formatting a score.
 * 
 * @author Dan Ruscoe
 */
public class ScoreView extends LinearLayout {
	...
	public ScoreView(Context context, Score score) {
		...
		Log.d(Constants.APP_LOG_NAME, "Displaying score for category ID "
				+ score.getCategoryId() + ": " + score.getCategoryName());

		mQuestionsAnswered.setText(getFormattedQuestionsAnsweredString(
				score.getCorrectAnswers(), score.getQuestionsAnswered()));

		mPercentCorrect.setText(getFormattedPercentCorrectString(
				score.getCorrectAnswers(), score.getQuestionsAnswered()));

		addView(mCategoryName, new RelativeLayout.LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
		addView(mQuestionsAnswered, new RelativeLayout.LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
		addView(mPercentCorrect, new RelativeLayout.LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
	}
	...
	public void setQuestionsAnswered(int correctAnswers, int questionsAnswered) {
		this.mQuestionsAnswered.setText(getFormattedQuestionsAnsweredString(
	...
	private String getFormattedPercentCorrectString(int correctAnswers,
	...
	private String getFormattedQuestionsAnsweredString(int correctAnswers,
			int questionsAnswered) {
		String questionsAnsweredText;

		if (questionsAnswered > 0) {
			questionsAnsweredText = mContext.getString(R.string.answered) + " "
					+ correctAnswers + " " + mContext.getString(R.string.of)
					+ " " + questionsAnswered + " "
					+ mContext.getString(R.string.questions_correctly);
		} else {
			questionsAnsweredText = mContext
					.getString(R.string.no_questions_answered);
		}
